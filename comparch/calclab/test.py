...
class CmdRes:
  ...
  def __eq_num(self, other_cmd, num):
    return (
      ...
      functools.reduce(lambda acc,y: acc and
                       (not y is None) and 
                       len(y) == 2 and
                       len(y[0]) >= num and
                       len(y[1]) >= num and
                       (not y[0][num] is None) and
                       (not y[1][num] is None) and
                       y[0][num].lower() == y[1][num].lower(),
...
def parse_output(output, test_case):
  ...
  re_prompt_txt = r'\s*([0-9]+) > ([a-zA-Z0-9\s\_\+><\-\*\^\|&!\(\)\_ ]+)'
  re_prompt = re.compile(re_prompt_txt)

  re_bin_txt = r'\s*0b([0-1_]*)[\s]+0x\s*([0-9a-fA-F_]*)\s*\(unsigned: (\s*[0-9]+\s*), signed: (\s*[\-0-9]+\s*)\)'
  re_bin = re.compile(re_bin_txt)

  re_flag_txt = r'\s*([CFSO]F)\s*=\s*([0-1]*)\s*'
  ...
  for l in output.splitlines():
    ...
    if new_prompt:
      ...
      # FLAG
      if (l.startswith("CF") or l.startswith("ZF") or
          l.startswith("SF") or l.startswith("OF")):
        res = re_flag.match(l)
        if (res):
          flag = res.group(1)
          val =  res.group(2)
...
def print_cmds(expected,result,
  ...
  if (len(expected) != len(result)):
    raise CmdException("Found %d commands instead of %d commands in your output! (check you are not changing the prompt string)" % (len(result), len(expected)))

  for exp,res in zip(expected, result):
    if (exp.cmd != res.cmd):
      raise CmdException("Found the command %s instead of %s! (check you are not changing the prompty string)" % (res.cmd, exp.cmd))
  ...
  for (testfile, mask, msg) in tests_to_run:
    with open(testfile, "r") as f:
      ...
      try:
        ...
        print(f"Something went wrong parsing your output: {e}, code: {e.code}")
        all_pass = False
        if stop_at_first:
          sys.exit(1)
      except CmdException as e:
        print(f"Something went wrong: {e}, code: {e.code}")
        all_pass = False
        if stop_at_first:
          sys.exit(1)

      if not all_pass:
        print(f"\nWARNING: Some test did not pass! Check the output.")
        sys.exit(2)
      else:
        print(f"\nAll the test you asked to run passed.")
        sys.exit(0)
