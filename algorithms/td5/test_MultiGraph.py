...
def test_contract():
    print("\nTesting 'contract'")
    Lt = [5, [['a', 'b', 1], ['a', 'c', 1], ['a', 'e', 1], [
        'b', 'c', 1], ['b', 'e', 2], ['c', 'e', 1], ['c', 'd', 1]]]
    Lt_cont_eb = [4, [['a', 'e', 2], ['a', 'c', 1],
                      ['c', 'e', 2], ['c', 'd', 1]]]
    Lt_cont_cd = [4, [['a', 'b', 1], ['a', 'c', 1], [
        'a', 'e', 1], ['b', 'c', 1], ['b', 'e', 2], ['c', 'e', 1]]]
    m = sol.MultiGraph(Lt)
    m2 = sol.MultiGraph(Lt_cont_eb)
    # print("Multigraph middle of Figure 3")
    # m.display()
    print(
        "** Contraction of {e, b}. Should obtain the multigraph on the left of Figure 5 (if no error message then result is correct)")
    m.contract('e', 'b')
    same_graph(m, m2)
    # m.display()
    m = sol.MultiGraph(Lt)
    ...
    print(
        "** Contraction of {c, d}. Should obtain the multigraph on the right of Figure 5 (if no error message then result is correct)")
    m.contract('c', 'd')
    # m.display()
    same_graph(m, m2)
...
def test_random_element():
    ...
    err = 0.05
    if not (abs(na / ntrials - 2 / 7) <= err and abs(nb / ntrials - 1 / 7) <= err and abs(nc / ntrials - 4 / 7) <= err):
...
def test_random_edge():
    ...
    for _ in range(ntrials):
        ...
        if (u == 'b' and v == 'c') or (u == 'c' and v == 'b'):
            nbc += 1
    print(
        "  Frequency of {a, b} (should be close to 0.166): " + str(nab / ntrials))
    print(
        "  Frequency of {a, c} (should be close to 0.333): " + str(nac / ntrials))
    print(
        "  Frequency of {b, c} (should be close to 0.5): " + str(nbc / ntrials))
    err = 0.05
    if not (abs(nab / ntrials - 1 / 6) <= err and abs(nac / ntrials - 1 / 3) <= err and abs(nbc / ntrials - 1 / 2) <= err):
        print("    Your code seems to not have the right frequencies.")
    else:
        print("    Your code seems to have the right frequencies.")
...
def test_random_cut():
    ...
    for _ in range(ntrials):
        ...
        if c == 3 and (len(L) == 1 and 'a' in L) or (len(L) == 3 and 'a' not in L):
            ...
            nac += 1
    print(
        "  Frequency of {a} U {b, c, d} (should be close to 0.1): " + str(na / ntrials))
    print(
        "  Frequency of {b} U {a, c, d} (should be close to 0.233): " + str(nb / ntrials))
    print(
        "  Frequency of {c} U {a, c, d} (should be close to 0.233): " + str(nc / ntrials))
    print(
        "  Frequency of {d} U {a, b, c} (should be close to 0.1): " + str(nd / ntrials))
    print(
        "  Frequency of {a, b} U {c, d} (should be close to 0.1): " + str(nab / ntrials))
    print(
        "  Frequency of {a, c} U {b, d} (should be 0): " + str(nac / ntrials))
    print(
        "  Frequency of {a, d} U {b, c} (should be close to 0.233): " + str(nad / ntrials))
    err = 0.05
...
def test_mincut_karger():
    ...
    for i in range(15, 22):
        ...
        print("     " + str(sol.mincut_karger(L, 0.1)))
