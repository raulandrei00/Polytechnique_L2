...
combine1_sky1 = [(1,5), (5, 2), (10, 3), (11, 0)]
combine1_sky2 = [(2,3), (3,7), (6, 1), (8, 2), (12, 5), (13, 0)]
combine1_res = [(1,5), (3, 7), (6, 2), (10, 3), (11, 2), (12, 5), (13, 0)]

combine2_sky1 = [(1, 11), (3, 13), (9, 0), (12, 7), (16, 0)]
combine2_sky2 = [(14, 3), (19, 18), (22, 3), (23, 13), (29, 0)]
combine2_res = [(1, 11), (3, 13), (9, 0), (12, 7), (16, 3), (19, 18), (22, 3), (23, 13), (29, 0)]

combine3_sky1 = [(1, 10), (3, 5), (8, 0)]
combine3_sky2 = [(1,20), (3, 2), (10,0)]
combine3_res = [(1, 20), (3, 5), (8, 2), (10, 0)]

def test_height_at():
    print("testing rects_height_at...\t", end="")
    ...
    for (x,h) in heights1:
        ...
        if res != h:
            print(f"\n your function rects_height_at returned the wrong result on {rects1}:")
            print(f" at position x={x} it returned a height of {res} instead of {h}")
            return 
    print("done.")

def test_simplify():
    print("testing simplify_skyline...\t", end="")
    for (sky_red, sk) in [(sky1_red, sky1)]:
        ...
        if res is None:
            ...
            return
        if res != sk:
            print(f"\n your function simplify_skyline returned the wrong result on {sky_red}:")
            print(f" it returned {res}")
            print(f" instead of  {sk}")
            assert not grading_mode
            return
    print("done.")


def test_skyline(name, fn, indent=1):
    textindent = '\t'*indent
    print(f"testing {name}...{textindent}", end="")
    # test run
    for rects, sky in [(rects1, sky1), (rects2, sky2), (rects3, sky3), (rects4, sky4), (rects5, sky5)]:
        res = fn(rects)
        if res is None:
            print(f"***** skipping {name} (unimplemented)")
            assert not grading_mode
            return False
        if res != sky:
            print(f"\n your function {name} returned the wrong result on {rects}:")
            print(f" it returned {res}")
            print(f" instead of  {sky}")
            assert not grading_mode
            return False
    print("done.")
    return True
...
def test_skyline_combine():
    print("testing combine_skylines...\t", end="")
    # test run
    for sky1, sky2, res in [(combine1_sky1, combine1_sky2, combine1_res), (combine2_sky1, combine2_sky2, combine2_res),
                            (combine3_sky1, combine3_sky2, combine3_res)]:
        tmp = sky.combine_skylines(sky1, sky2)
        if tmp is None:
            print("***** skipping combine_skylines (unimplemented)")
            assert not grading_mode
            return
        if res != tmp:
            print("\n your function combine_skylines returned the wrong result on:")
            print(f" S1 = {sky1}")
            print(f" S2 = {sky1}")
            print(f" it returned {tmp}")
            print(f" instead of  {res}")
            assert not grading_mode
            return False
    print("done.")
...
def test_skyline_dac():
    ...
    for i in range(10, 500, 50):
        ...
        print(f" time (n={n}): {end - start}")
        if end-start >= 10:
            print(f" Your DAC function is probably correct, but took more than 10 seconds on an example of size {n}, so your complexity is probably not better than O(n^2)")
            assert not grading_mode
            return
    print("done.")
