...
def test_generic(f,inputs,outputs,name=""):
    for i in range(len(inputs)):
        ...
        if y2 is None:
            ...
            return
        if y1 != y2:
            print("!! Error: your function {} returned the wrong answer ({} instead of {}) on {}".format(name,y2, y1, x))
            return
    ...
    for i in range(I):
        for j in range(J):
            if peak.is_peak(M,i,j):
                res+=[(i,j)]
    if res!=output:
        print("!! Error in your function is_peak: on matrix")
        print(M)
        print("the list of peaks is ", output)
        print("but your algorithm returned ", res)
        return
    print("++ Test success for is_peak.")
