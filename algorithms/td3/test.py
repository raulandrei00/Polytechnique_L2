...
def compare_rec_while(size, nb_tests):
    ...
    print(f"Average time: recursive = {total_rec/nb_tests}, iterative = {total_while/nb_tests}")
...
def compare_binary_ternary(n):
    ...
    implemented_BR = cost_binary_search_value(N,1) is not None
    implemented_TR = cost_ternary_search_value(N,1) is not None
    ...
    if implemented_TR or implemented_BR:
        for i in range(1, n):
            ...
            for _ in range(nb_tries):
                ...
                A.sort()
                v = random.choice(A)
                if implemented_BR: brcost += cost_binary_search_value(A,v)
                if implemented_TR: trcost += cost_ternary_search_value(A,v)
...
def compare_all(size, nb_tests):
    ...
    print(f"""Average execution times:
        binary        = {total_b/nb_tests},
        ternary       = {total_t/nb_tests},
        exponential   = {total_e/nb_tests},
        interpolation = {total_i/nb_tests}""")
...
def test_searches(name, func):
    ...
    for i in range(nb_tests):
        ...
        res = func(A,v)
        if (res != j):
            print(f"{name}: wrong result for A={A} and v={v}: it should be {j} but is {res}")
            assert not grading_mode
            return False
    return True
...
def test5():
    ...
    if any_unimplemented([binary_search, ternary_search, exponential_search, interpolation_search]):
        ...
        return
    size = 100000
    print(f'comparing on arrays of size {size}...', end = " ", flush = True)
    compare_all(size,20)
...
def test6():
    ...
    def test_first_last (name, func, A, k, correct):
        ...
        if result == correct: 
            ...
            print (f"wrong {name} for {k} in {A}: {result} instead of {correct}")
            return False
...
def test7():
    ...
    def test_closest(A, v, k, correct):
        ...
        if result == correct:
            ...
            print(f"wrong {k} closest elements for {v} in {A}: {result} instead of {correct}")
            return False
...
def test8():
    ...
    def test_frequency(A, result):
        ...
        if freq is None:
            ...
            print(f"""wrong frequencies for {A}: 
            {freq} instead of 
            {result}"""
            )
            return False
