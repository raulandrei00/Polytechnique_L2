...
(* Nat also  provides a test of equality *)
Eval compute in (3 =? 4).

(* you can prove it correctly tests equality:  *)
...
Fixpoint sorted (l : list nat) :=
  ...
  | cons _ (nil _) => true
  | cons n l' =>
      if le_head n l' then sorted l' else false
  end.
...
(* What is the complexity of insertion_sort ? *)
(* answer in the comment  O(N^2) time and O(N) space *)
...
(* Implement elem so that elem n l asserts that n is an element in l *)
Fixpoint elem (n:nat) (l : list nat) : Prop :=
  match l with
  | nil _ => False
  | cons m l' => m = n \/ elem n l'
  end.

Lemma ins_elem1 : forall l n m,
  elem m l -> elem m (insert n l).
Proof.
elim=> [| a l IH] n m /=.
- (* l = [] *) by [].
- (* l = a :: l *)
...
- (* l = a :: l *)
  destruct (n <=? a) eqn:E.
  + (* n <= a, inserted before a *)
    simpl. left. reflexivity.
  + (* n > a, inserted later in the tail *)
...
Proof.
elim=> [| a l IH] n /=.
- (* l = [] *) move=> H. contradiction.  (* elem n [] = False *)
- (* l = a :: l *)
  move=> H.
  destruct H as [H_eq | H_in].
  + (* n = a *)
    simpl. rewrite H_eq. apply ins_elem2.
  + (* n ∈ l *)
    simpl. apply ins_elem1.
    apply IH. assumption.
Qed.
...
elim=> [| a l IH] n /=.
- (* l = [] *) 
  simpl. move=> H. contradiction.  (* elem n [] = False *)
- (* l = a :: l *)
  simpl. move=> H.
  (* insertion_sort (a :: l) = insert a (insertion_sort l) *)
  apply ins_elem3 in H.
  destruct H as [H_eq | H_in].
  + (* n = a *)
...
No, it just shows the list preserves its elements
*)
...
Proof.
move=> l n Hsorted.
have [Hs _] := ins_sorted_aux l n Hsorted.
exact Hs.
...
- trivial.
- (* l = n :: l *)
  (* insertion_sort (n :: l) = insert n (insertion_sort l) *)
  apply ins_sorted.
  exact IH.
Qed.
...
(* The following property corresponds to le_head, but for trees: *) 
...
Fixpoint to_list (t : tree) :=
  match t with
  | Leaf => nil _
  | Node n t1 t2 => cons n (merge (to_list t1) (to_list t2))
end.
